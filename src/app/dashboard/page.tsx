...
{loadingItems ? (
<div className="text-sm text-gray-600">LoadingÎ“Ã‡Âª</div>
) : items.length === 0 ? (
...
{it.issuer ? <> â”¬â•– {it.issuer}</> : null}
{it.identifier ? <> â”¬â•– {it.identifier}</> : null}
</div>
</div>
...
<> â”¬â•– Window: {it.renewal_window_days}d</>
) : null}
</div>
</li>
))}
...
</div>    <div className="text-xs text-gray-600">{r.state} Î“Ã‡Ã³ {r.trade} Î“Ã‡Ã³ {r.requirement_type}</div>  </div></li>
))}
</ul>
)}
</div>
