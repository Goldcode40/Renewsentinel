...
const ui = {
  ...
  grid: "grid gap-4",
  input: "w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200",
  select: "w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200",
  textarea: "w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200",
  buttonPrimary: "inline-flex items-center justify-center rounded-xl bg-slate-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50",
  buttonSecondary: "inline-flex items-center justify-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50",
  buttonGhost: "inline-flex items-center justify-center rounded-xl px-3 py-2 text-sm font-medium text-slate-600 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50",
...
async function loadOrgs(nextUserId?: string) {
...
if ((json.orgs?.length ?? 0) === 0) {
  const resCreate = await fetch("/api/org/bootstrap", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      user_id: uid,
  ...
  if (!created?.ok) {
    setErr(created?.error ?? "Failed to create organization")
    return
  }

  const resReload = await fetch(`/api/orgs?user_id=${uid}`, { cache: "no-store" })
  const reloadJson = await resReload.json()
  if (!reloadJson?.ok) {
    setErr(reloadJson?.error ?? "Failed to reload orgs")
    return
  }

  setOrgs(reloadJson.orgs ?? [])
  if ((reloadJson.orgs?.length ?? 0) > 0) {
    setOrgId(reloadJson.orgs[0].id)
  }
  return
}
          ...
          <div className="flex h-full flex-col justify-between gap-4">
            <div className="text-center">
              ...
              <div className="mt-2 text-xs text-gray-500">
                PDF evidence pack with compliance items, insurance, subcontractors, and attached proof.
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <button
                className={cx(ui.buttonSecondary, "w-full")}
                onClick={() => {
                  if (!orgId || !isPremium) return
              ...
              >
                View Proof Pack Data
              </button>

              {!isPremium ? (
                <div className="text-center text-[11px] text-blue-700">
                  Upgrade or start trial to unlock exports.
                </div>
              ) : null}
            </div>
          </div>
...
<section className={cx(ui.section, "space-y-4 overflow-hidden")}>
  <div className="flex flex-col gap-4 border-b border-slate-200 px-5 py-5 md:flex-row md:items-start md:justify-between">
    <div className="max-w-2xl">
      ...
      <h2 className="mt-3 text-2xl font-semibold tracking-tight text-slate-900">We set it up for you</h2>
      <p className="mt-2 text-sm leading-6 text-slate-600">
        Submit your request and upload any supporting docs. We will configure your requirements and tracking so you can get compliant faster.
      </p>
    </div>

    <div className="grid gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700 min-w-[280px]">
      <div className="flex items-center justify-between gap-3">
        <span className="text-xs font-medium uppercase tracking-wide text-slate-500">Organization</span>
        <span className="font-mono text-xs text-slate-700">{orgId || "(none)"}</span>
      </div>
      <div className="flex items-center justify-between gap-3">
        <span className="text-xs font-medium uppercase tracking-wide text-slate-500">Viewer role</span>
        <span className="text-xs font-medium text-slate-700">{concierge.viewer_role || "unknown"}</span>
      </div>
      <div className="flex items-center justify-between gap-3">
        <span className="text-xs font-medium uppercase tracking-wide text-slate-500">Status</span>
        {concierge.request?.status ? (
          <span className={`rounded-full border px-2.5 py-1 text-[11px] font-semibold ${badgeForStatus(concierge.request.status)}`}>
...
{o.name} ({o.role})
              ...
              {showUpgrade ? (
  <div className="mb-4 rounded-2xl border border-blue-200 bg-blue-50/80 p-5 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
